...
- **Linux**: Run the single installer script in the root directory:
  ```bash
  bash install-linux.sh
  ```
  Options: `--run` (start immediately), `--uninstall` (clean removal), `--with-drm-capsule`, `-y` (non-interactive).
- **Windows**: Launch the graphical installer executable:
  - Run `Aster-Browser-Setup.exe` directly. A visual installer window will guide you through directory selection, desktop/start menu shortcut creation, and installation.
