...
public final class CompatibilityTests {
    ...
    private static void bodies(ScriptSession js,AtomicReference<String> type,AtomicReference<String> body)throws Exception{
        ...
        yes(js,"(()=>{let text='a🌎b'.repeat(6000),bytes=new TextEncoder().encode(text),d=new TextDecoder(),parts=[];for(let i=0;i<bytes.length;i+=4091)parts.push(d.decode(bytes.subarray(i,i+4091),{stream:true}));parts.push(d.decode());return parts.join('')===text&&new TextDecoder().decode(bytes)===text;})()","UTF-16 output chunks split a surrogate or lost streaming text");
        yes(js,"(()=>{let bad=new Uint8Array([0xed,0xa0,0x80,0xf0,0x9f]);if(new TextDecoder().decode(bad)!=='����')return false;try{new TextDecoder('utf-8',{fatal:true}).decode(bad);return false;}catch(e){return e.name==='TypeError';}})()","Buffered decoder changed invalid UTF-8 handling");
