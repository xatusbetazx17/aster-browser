...
def desktop(test=False, package=False):
    ...
    if test:
        ...
        run("java", "-Djava.awt.headless=true", "-cp", os.pathsep.join(map(str, [classes, tests]))+os.pathsep+libraries, "io.aster.desktop.ResponsiveTests", BUILD)
        run("java", "-Djava.awt.headless=true", "-cp", os.pathsep.join(map(str, [classes, tests]))+os.pathsep+libraries, "io.aster.desktop.FlexLayoutTests", BUILD)
