...
final class ResourceLoader {
    ...
    private static void copy(URI page,URI initial,int limit,boolean script,OutputStream out,SiteData.Request request) throws IOException {
        ...
        for(int redirects=0;redirects<=5;redirects++) {
            ...
            c.setInstanceFollowRedirects(false); c.setConnectTimeout(8000); c.setReadTimeout(8000); c.setRequestProperty("Accept-Encoding","identity"); c.setRequestProperty("User-Agent","AsterEnginePreview/0.5");
            request.prepare(c);
            try {
