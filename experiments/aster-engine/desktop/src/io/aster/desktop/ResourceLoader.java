...
final class ResourceLoader {
    ...
    static String script(URI page,String src) throws IOException {
        ...
        ByteArrayOutputStream out=new ByteArrayOutputStream(); copy(page,uri,1_000_000,true,out); return new String(out.toByteArray(),StandardCharsets.UTF_8);
    }
    static Path media(URI page,URI uri) throws IOException {
        ...
        try(OutputStream out=Files.newOutputStream(file)) { copy(page,uri,64*1024*1024,false,out); return file; }
        catch(Throwable e) { Files.deleteIfExists(file); throw e; }
    }
    private static void copy(URI page,URI initial,int limit,boolean script,OutputStream out) throws IOException {
        ...
        for(int redirects=0;redirects<=5;redirects++) {
            ...
            c.setInstanceFollowRedirects(false); c.setConnectTimeout(8000); c.setReadTimeout(8000); c.setRequestProperty("Accept-Encoding","identity"); c.setRequestProperty("User-Agent","AsterEnginePreview/0.2");
            try {
