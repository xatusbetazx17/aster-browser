...
final class HLSConnectionHolder extends ConnectionHolder {
    ...
    private void adjustBitrate(long readTime) {
        // Aster patch: a cached/loopback segment can finish within one clock tick.
        // Keep the current variant when the timing or length is not measurable.
        if (readTime <= 0 || urlConnection.getContentLength() <= 0) {
            return;
        }
        long measuredBitrate = (long) urlConnection.getContentLength() * 8000 / readTime;
        int avgBitrate = (int) Math.min(Integer.MAX_VALUE, measuredBitrate);
